...
class ClientActor extends Actor with ActorLogging {
  ...
  var serverActor = context.system.actorFor("akka://ServerSys@127.0.0.1:2552/user/serverActor")
  // Second option - create remote Actor instance
  val addr = Address("akka", "ServerSys", "127.0.0.1", 2552);
  serverActor = context.system.actorOf(Props[ServerActor].withDeploy(Deploy(scope = RemoteScope(addr))))
  //Third option - creating the actor using the actor name 
  //defined via application.conf
  serverActor = context.system.actorOf(Props[ServerActor], name = "remoteServerActor")

  def receive: Receive = {
    case message: String =>
      implicit val timeout = Timeout(5 seconds)
      val future = serverActor ? message
